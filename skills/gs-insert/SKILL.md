...
### 2. Show the stack with insertion positions
...
### 3. Ask for the insertion position
...
### 4. Determine the new branch name
...
### 5. Create the new branch from `<branch-above>`
...
### 6. Handle staged/unstaged changes

**Follow the [safe staging flow](../../docs/staging.md) pattern.** Show the user what will be staged, warn on sensitive files, and stage by name. If there are no changes, ask whether to create an empty branch (skip step 7, continue with metadata update) or cancel; on cancel, undo with `git checkout <branch-above> && git branch -D <new-branch>`.
...
### 10. Verify affected branches

Run the project's verification commands as defined in CLAUDE.md (e.g., `pnpm typecheck`, `pnpm lint`) on each affected branch (the new branch and all rebased downstream branches).
